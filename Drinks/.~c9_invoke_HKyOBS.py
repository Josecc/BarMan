...
def search_bartender(intent, session):
    ...
    if 'Drink' in intent['slots']:
        input _drink = intent['slots']['Drink']['value']
        res = requests.get("http://www.thecocktaildb.com/api/json/v1/1/search.php?s=margarita").json()
        #instructions = res["strDrink"]
        speech_output = "Here are the instructions for " + \
                        input_drink  + \
                        ":\n" + \
                        instructions
